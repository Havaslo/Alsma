...
export const createAiAgentService = (options: AgentOptions) => {
  ...
  const replyNow = async (
    ...
    const prompt = [
      ...
      "Сначала ответь именно на заданный вопрос, а затем поддержи разговор: при необходимости предложи уместный следующий шаг или задай один короткий уточняющий вопрос. Не заставляй каждый ответ заканчиваться вопросом или действием и не повторяй описание SPA, если оно уже было дано. Если гость выражает общий интерес, помоги выбрать из двух-трёх подходящих форматов (проживание, SPA на день, процедуры), а не выдавай без запроса длинный список услуг.",
