...
Сервис бронирования обогащает предложения данными `hotel-definitions` Eptera: фото, площадь, вместимость, количество комнат, варианты кроватей и описание. Эти данные кэшируются на пять минут, остаются на backend и передаются браузеру только как часть ответа `/api/booking/offers`. При отсутствии отдельных полей frontend показывает нейтральные значения.
