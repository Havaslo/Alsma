  ...
  return (
    <div className="relative" ref={containerRef}>
      ...
      <button
        ...
        className={cn(
          "flex field-control items-center justify-between gap-3 text-left hover:border-border",
          className,
        )}
        disabled={disabled}
        onClick={(event) => {
      ...
      <div
        aria-hidden={!isOpen}
        className={cn(
          "absolute z-30 mt-2 w-full origin-top rounded-2xl border border-border/70 bg-surface/95 p-1.5 text-surface-foreground shadow-lg shadow-foreground/5 backdrop-blur-xl transition-[opacity,transform] duration-150",
          isOpen
        ...
        {options.map((option) => {
          ...
          return (
            <button
              ...
              className={cn(
                "group flex min-h-10 w-full items-center gap-3 rounded-xl px-3 py-2 text-left text-sm transition-colors outline-none focus-visible:ring-4 focus-visible:ring-ring/15 disabled:cursor-not-allowed disabled:opacity-50",
                isSelected
                  ? "bg-primary/10 font-medium text-primary hover:bg-primary/15"
                  : "hover:bg-primary/5 hover:text-surface-foreground focus-visible:bg-primary/5",
              )}
