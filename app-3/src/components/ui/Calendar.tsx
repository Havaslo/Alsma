...
export const Calendar = ({
  ...
  return (
    <section
      aria-label="Calendar"
      className={cn(
        "w-full rounded-3xl border border-border/70 bg-surface/80 p-4 text-surface-foreground shadow-sm shadow-foreground/5",
        className,
      )}
    >
      <header className="mb-4 flex items-center justify-between gap-3">
        <button
          ...
          className="grid size-9 place-items-center rounded-xl border border-border/70 bg-surface/70 shadow-sm transition hover:bg-muted/70 focus-visible:ring-4 focus-visible:ring-ring/15 focus-visible:outline-none"
          onClick={() => moveMonth(-1)}
          type="button"
        >
          <ChevronLeft aria-hidden="true" className="size-4" />
        </button>
        <h2 aria-live="polite" className="font-primary text-sm font-semibold">
          {monthLabel}
        </h2>
        <button
        ...
        {days.map((date) => {
          ...
          return (
            <button
              ...
              className={cn(
                "aspect-square rounded-xl text-sm transition-colors focus-visible:ring-4 focus-visible:ring-ring/15 focus-visible:outline-none",
                isSelected
