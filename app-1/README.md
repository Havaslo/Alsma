...
- `src/routes` contains file-based route declarations; `src/routeTree.gen.ts` is generated from the route files.
...
Экран бронирования (`src/pages/BookingPage.tsx`) позволяет выбрать диапазон дат в одном календаре и до двух номеров. Для каждого номера задаётся независимый состав гостей: количество взрослых и строка детей с вариантом «Нет» по умолчанию, возрастом и количеством. Открытый список гостей и номеров закрывается кнопкой «Готово» или кликом вне списка. Календарь показывает ориентировочную минимальную цену за ночь и индикатор скидки для каждого дня. В сводке каждый номер отображается отдельной карточкой с составом гостей и выбранной категорией. Выбор номера и тарифа проходит последовательно: сначала номер 1, затем номер 2; выбранная категория отмечается у соответствующего набора людей. При поиске составы объединяются в параметры `adults` и `childAges`, а `roomCount` отправляется в `/api/booking/offers` и `/api/booking/reservations`; итоговая стоимость отображается с учётом выбранного количества номеров.
