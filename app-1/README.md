...
Экран бронирования (`src/pages/BookingPage.tsx`) позволяет выбрать диапазон дат в одном календаре и до двух номеров. Для каждого номера задаётся независимый состав гостей: количество взрослых и список детей с возрастом. При поиске составы объединяются в параметры `adults` и `childAges`, а `roomCount` отправляется в `/api/booking/offers` и `/api/booking/reservations`; итоговая стоимость отображается с учётом выбранного количества номеров.
