...
В административной форме проживания превью фотографий отображаются в квадратных контейнерах с кадрированием `object-cover`. Первое фото по-прежнему используется как обложка, а порядок фотографий можно менять стрелками. На публичной странице номера по нажатию на фото открывается полноразмерный просмотр в модальном окне.

## Единые сценарии отдыха

Блок «Каким будет ваш идеальный отдых!» на главной странице и блок «Готовые сценарии отдыха» на странице акций используют одну коллекцию `offers/ready-scenarios`. Редактирование сценариев доступно в административном разделе «Акции» → «Готовые сценарии»; отдельного редактора сценариев в разделе «Главная» нет. Переносы строк в описании, введённые в многострочном поле админки, сохраняются и отображаются в карточке. Строки с префиксом `-` или `•` отображаются как стилизованный список. Изображение карточки растягивается на всю высоту карточки при увеличении описания.
...
## Данные номеров в онлайн-бронировании

Карточки номеров на странице `/booking` получают из Eptera галерею фотографий, площадь, вместимость, количество комнат и описание через запрос hotel definitions. Изображение в каталоге заполняет левую колонку карточки без внутренних отступов. Кнопка «Подробнее» открывает двухколоночную карточку номера без отдельной шапки: слева — увеличенное основное фото и миниатюры галереи, справа — название, характеристики, спальные места, полное описание и CTA «Выбрать номер». Если Eptera не передала отдельное поле, интерфейс показывает нейтральное значение по умолчанию.

## Последовательный выбор при нескольких номерах

Подтверждение бронирования использует тот же набор поиска, по которому были выбраны номера и тарифы: актуальные даты, состав гостей и количество номеров. Это важно для многономерного запроса в Eptera — в запрос не подставляется устаревшее начальное значение `roomCount: 1`. Тариф для каждой позиции должен быть выбран до отправки формы.

При поиске нескольких номеров бронирование разделено на два последовательных этапа. Сначала гость выбирает категории номеров для всех позиций: после выбора первого номера остаётся на этапе номеров и выбирает второй, третий и т. д. Только после заполнения всех категорий открывается этап тарифов. На нём тариф выбирается отдельно для каждого уже выбранного номера, по очереди. Кнопки «Номер 1», «Номер 2» и сводка справа позволяют переключаться между позициями. К форме контактов пользователь переходит только после выбора тарифа для каждой позиции. Карточки тарифов показывают преимущества тегами и имеют раскрываемый блок с подробным описанием и условиями отмены.
