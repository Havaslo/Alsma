...
`BACKEND_PROXY_TARGET` is a shared project variable and must point to the registered backend application using its internal runtime address (`http://app-{app-2 UUID without hyphens}:{app-2 port}`). If this variable is changed, publish `app-1` (or use **Update all**) so the production static server receives the new runtime configuration; changing browser cache cannot update this proxy target.
