...
При поиске нескольких номеров бронирование проходит последовательно для каждой позиции: сначала гость выбирает категорию номера, затем отдельный тариф для этого номера. После выбора тарифа интерфейс явно показывает кнопки «Номер 1», «Номер 2» и т. д. — незаполненная позиция ведёт к выбору категории, а заполненная позволяет изменить её тариф. В сводке справа каждая позиция также является переключателем и показывает выбранные категорию и тариф. Кнопка выбора тарифа автоматически переводит к следующему незаполненному номеру, а к контактам — только после заполнения всех позиций. Карточки тарифов показывают преимущества тегами и имеют раскрываемый блок с подробным описанием и условиями отмены.
