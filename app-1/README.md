...
Карточки номеров на странице `/booking` получают из Eptera фотографию, площадь, вместимость, количество комнат и описание через запрос hotel definitions. Если Eptera не передала отдельное поле, интерфейс показывает нейтральное значение по умолчанию. Кнопка «Подробнее» открывает полную карточку номера в модальном окне.
