  ...
  return (
    <div className="space-y-6">
      <section>
        ...
        <h1 className="mt-2 text-3xl font-semibold text-brand">SPA</h1>
        <p className="mt-2 text-sm leading-6 text-muted-ui-foreground">
          Настройте каждый блок страницы SPA в отдельном разделе.
        </p>
      </section>
      <nav
        aria-label="Разделы страницы SPA"
      ...
      {tab === "massages" && (
        <AdminSpaCollectionForm
          {...common}
          defaults={defaults.massages}
          fields={[
            { key: "name", label: "Процедура" },
            { key: "duration", label: "Длительность" },
            { key: "price", label: "Стоимость" },
          ]}
          addItem={{ name: "", duration: "", price: "" }}
          addLabel="Добавить процедуру"
          layout="table"
          itemKey="massages"
          title="Массажные процедуры"
          description="Заполните строки таблицы с процедурами, длительностью и стоимостью."
        />
      )}
      {tab === "promotions" && (
        <AdminSpaCollectionForm
          {...common}
          defaults={defaults.promotions}
          fields={[
          ...
          addLabel="Добавить акцию"
          itemKey="promotions"
          title="Актуальные акции"
          description="Управляйте карточками актуальных предложений SPA."
        />
      )}
      {tab === "menu" && (
        <AdminSpaCollectionForm
          ...
          description="Добавляйте блюда, напитки и услуги кафе."
        />
      )}
      {tab === "additional" && (
        <AdminSpaCollectionForm
          {...common}
          defaults={defaults.additional}
          fields={[
          ...
          title="Дополнительные услуги и посещение SPA"
          description="Заполните таблицу дополнительных услуг и тарифов посещения."
        />
      )}
    </div>
  );
