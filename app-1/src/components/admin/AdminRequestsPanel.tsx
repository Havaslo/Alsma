...
const typeOptions: { label: string; value: RequestType }[] = [{ label: "Все типы", value: "all" }, { label: "Звонки", value: "call" }, { label: "Чаты", value: "chat" }];
const requestType = (item: { details: Record<string, unknown> }): Exclude<RequestType, "all"> => item.details.channelType === "call" ? "call" : "chat";
const requestTypeLabel = (type: Exclude<RequestType, "all">) => type === "call" ? "Звонок" : "Чат";
...
export const AdminRequestsPanel = () => {
  ...
  const items = useMemo(() => {
    ...
    return (requests.data?.items ?? []).filter((item) => {
      const haystack = [item.title, item.description, item.requester, item.contact, item.category, item.details.source].join(" ").toLocaleLowerCase("ru-RU");
      return (status === "all" || item.status === status) && (type === "all" || requestType(item) === type) && (!term || haystack.includes(term));
    });
  }, [requests.data?.items, search, status, type]);

  return <div className="space-y-5"><section className="grid gap-4 md:grid-cols-[minmax(20rem,1fr)_12rem_12rem]"><label><span className="mb-2 block text-sm font-medium text-muted-ui-foreground">Поиск</span><span className="flex min-h-12 items-center gap-3 rounded-2xl border border-line bg-brand-foreground px-4"><Search className="size-4 text-muted-ui-foreground" /><input className="min-w-0 flex-1 bg-transparent text-sm outline-none placeholder:text-muted-ui-foreground" onChange={(event) => setSearch(event.target.value)} placeholder="Заголовок, клиент, контакт" value={search} /></span></label><label><span className="mb-2 block text-sm font-medium text-muted-ui-foreground">Тип обращения</span><DropdownSelect<RequestType> ariaLabel="Фильтр по типу обращения" onChange={setType} options={typeOptions} triggerClassName="min-h-12 rounded-2xl border border-line bg-brand-foreground px-4 text-sm" value={type} /></label><label><span className="mb-2 block text-sm font-medium text-muted-ui-foreground">Статус</span><DropdownSelect<StatusFilter> ariaLabel="Фильтр обращений по статусу" onChange={setStatus} options={statusOptions} triggerClassName="min-h-12 rounded-2xl border border-line bg-brand-foreground px-4 text-sm" value={status} /></label></section><section className="overflow-hidden rounded-3xl border border-line bg-brand-foreground"><p className="border-b border-line px-5 py-4 text-sm font-semibold">Всего обращений: {items.length}</p><div className="overflow-x-auto"><table className="w-full min-w-6xl border-collapse text-left text-sm"><thead className="bg-page text-xs font-semibold tracking-wide uppercase"><tr><th className="px-5 py-4">Источник</th><th className="px-5 py-4">Дата</th><th className="px-5 py-4">Клиент</th><th className="px-5 py-4">Обращение</th><th className="px-5 py-4">Статус</th><th className="px-5 py-4 text-right">Действие</th></tr></thead><tbody>{items.map((item) => { const currentType = requestType(item); return <tr className="border-t border-line align-top" key={item.id}><td className="px-5 py-4"><strong className="block font-semibold text-brand">{item.details.source ?? item.category}</strong><span className="mt-1 block text-xs text-muted-ui-foreground">{requestTypeLabel(currentType)}</span></td><td className="px-5 py-4 text-muted-ui-foreground">{new Date(item.createdAt).toLocaleString("ru-RU")}</td><td className="px-5 py-4"><strong className="block">{item.requester ?? "—"}</strong><span className="mt-1 block text-xs text-muted-ui-foreground">{item.contact ?? "—"}</span></td><td className="max-w-md px-5 py-4"><strong className="block">{item.title}</strong><span className="mt-1 block text-xs text-muted-ui-foreground">{item.description ?? "Без описания"}</span></td><td className="px-5 py-4"><span className={cn("inline-flex rounded-full px-3 py-1 text-xs font-semibold", statusTones[item.status])}>{statusLabels[item.status]}</span></td><td className="px-5 py-4 text-right"><Button className="px-3" onClick={() => navigate({ to: buildRoute(ROUTES.adminRequest, { requestId: item.id }) })} variant="secondary"><Eye className="size-4" /> Открыть</Button></td></tr>; })}</tbody></table></div>{requests.isLoading && <p className="p-12 text-center text-muted-ui-foreground">Загружаем обращения…</p>}{!requests.isLoading && !items.length && <p className="p-12 text-center text-muted-ui-foreground">Обращений пока нет.</p>}</section></div>;
};
