...
import { useAgentSettings, useSaveAgentSettings } from "@/lib/admin/useAgentScenarios";
...
export const AgentSettingsTabPanel = ({ tab }: { readonly tab: AgentSettingsTab }) => {
  ...
  return (
    <Form className="space-y-6" form={form} onSubmit={(values) => saveSettings.mutate(values)}>
      {tab === "general" && (
      ...
      {tab === "capabilities" && (
        <div className="space-y-4">
          <InfoCard icon={<ShieldCheck className="size-5" />} title="Разрешённые действия"><p>Агент может консультировать клиента и создавать обращение, но не имеет права самостоятельно создавать бронь.</p></InfoCard>
          ...
          <div className="rounded-2xl border border-line bg-page p-4"><CheckboxField checked={false} label="Создавать бронь" onChange={() => undefined} /><p className="mt-1 ml-8 text-sm text-muted-ui-foreground">Недоступно по правилам проекта. Клиенту можно отправить ссылку на инструмент бронирования.</p></div>
        </div>
      )}
      {tab === "data" && (
        <div className="space-y-4">
          <InfoCard icon={<Link2 className="size-5" />} title="Данные для заявки"><p>Собираем только сведения, необходимые для консультации и обратной связи. Срок хранения диалогов пока не задаём.</p></InfoCard>
      ...
      {tab === "notifications" && (
        <div className="space-y-5">
          <InfoCard icon={<BellRing className="size-5" />} title="Прозрачность ответа"><p>Клиент должен понимать, что получает ответ AI-агента, а не сотрудника отеля.</p></InfoCard>
      ...
      <div className="flex justify-end border-t border-line pt-5"><Button disabled={saveSettings.isPending} type="submit"><Save className="size-4" /> Сохранить настройки</Button></div>
    </Form>
  );
};
...
const InfoCard = ({ icon, title, children }: { readonly icon: ReactNode; readonly title: string; readonly children: ReactNode }) => <div className="rounded-2xl border border-line bg-page p-4"><div className="flex items-center gap-3 font-semibold text-brand">{icon}<span>{title}</span></div><div className="mt-3 text-sm leading-6 text-muted-ui-foreground">{children}</div></div>;
