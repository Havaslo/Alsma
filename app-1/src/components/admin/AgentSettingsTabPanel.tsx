...
export const AgentSettingsTabPanel = ({
  ...
  const showDisclosure = useWatch({
    control: form.control,
    name: "showAiDisclosure",
  });
  ...
  return (
    <Form
      className="space-y-6"
      form={form}
      onSubmit={() => toast.success("Настройки агента сохранены")}
    >
      {tab === "general" && (
      ...
      {tab === "data" && (
        <div className="space-y-4">
          ...
          <CheckboxField
            checked={useWatch({ control: form.control, name: "collectName" })}
            label="Имя клиента"
            onChange={(checked) => form.setValue("collectName", checked)}
          />
          <CheckboxField
            checked={useWatch({ control: form.control, name: "collectPhone" })}
          ...
          <CheckboxField
            checked={useWatch({
              control: form.control,
              name: "collectGuestsCount",
            })}
            label="Количество гостей"
            onChange={(checked) => form.setValue("collectGuestsCount", checked)}
          />
          <CheckboxField
            checked={useWatch({ control: form.control, name: "collectDates" })}
