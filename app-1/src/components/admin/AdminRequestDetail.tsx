...
const fallbackDetails = (requestId: string): MockRequestDetails => {
  ...
  return {
    ...
    transcript: [
      {
        author: "guest",
        text: request?.intent ?? "Новое обращение гостя",
      },
      {
        author: "manager",
        ...
        <section className="rounded-3xl border border-line bg-brand-foreground p-6 sm:p-8">
          <header className="border-b border-line pb-6">
            ...
            <p className="mt-2 text-sm text-muted-ui-foreground">
              Источник: {request.channelLabel} · Создано {request.createdAt} ·
              Срок: {details.deadline}
            </p>
          </header>
          ...
          <div className="mt-6 rounded-2xl border border-line bg-page/50 p-5">
            ...
            <p className="mt-3">{details.managerTask}</p>
          </div>
          ...
          <div className="mt-7 space-y-6">
            {details.transcript.map((message, index) => (
              <article
                className={cn(
                  "max-w-3xl rounded-3xl border border-line p-5",
                  message.author === "manager"
                    ? "ml-auto bg-panel"
                    : "mr-auto bg-brand-foreground",
                )}
          ...
          <div className="mt-7 rounded-2xl border border-line bg-page/50 p-5">
            <p className="text-xs font-semibold tracking-wider text-muted-ui-foreground uppercase">
              Итог обращения
            </p>
            <p className="mt-3">{details.outcome}</p>
          </div>
        </section>

        <aside className="space-y-5">
          <DetailsCard title="Информация о клиенте" values={details.client} />
          <DetailsCard title="Собранные данные" values={details.collected} />
        </aside>
      </div>
    </div>
  );
